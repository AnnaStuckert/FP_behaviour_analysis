...
```
```{r}
p_load(lmerTest, performance)
model_real <- lmer(light_preference ~ Week + (1 | ID), data = df)
summary(model_real)
performance::model_performance(model_real)
```
...
generate_mouse_data <- function(n_mice = 13, seed = 123, effect = TRUE) {
  set.seed(seed)
  offset <- rnorm(n_mice, 0, 6.744)

  pre_mean  <- 10.965293
  pre_sd    <- 5.919228	
  week1_mean <- 5.627495 #try with 15
  week1_sd   <- 4.338694	
...
generate_mouse_data <- function(n_mice = 13, seed = 123, effect = TRUE) {
  ...
  pre_mean  <- 10.965293
  pre_sd    <- 5.919228	
  week1_mean <- 15.627495 #try with 15
  week1_sd   <- 4.338694	
  week2_mean <- 21.735614	
  week2_sd   <- 5.083281	#tried with 5 instead of 20
